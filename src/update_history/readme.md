...
> 2、新增两篇文章：Enable two-factor authentication (2FA) - Github解决方案、docsearch的申请配置安装（src/document/other/）
...
## 2025-06-06:
> 1、新增了一篇文章，nodejs添加到前端文章中
> 2、运维官网新增官网内容及运维服务包下载地址（src/website-all.md）
...
## 2025-09-23:
> 1、更新了k8s单机部署文章（新增了更新证书等操作）
> 2、更新了kubeadm搭建k8s集群的一些错误信息；
...
> 6、markdown代码块颜色配置（src/.vuepress/styles/index.scss）
...
## 2026-02-02:
> 1、更新了git 1篇文章
> 2、新增Prometheus相关4篇文章
> 3、新增ubuntu apt 源 1篇文章
...
## 2026-02-05:
> 1、更新了《【云原生】PromQL 常用内置指标》文章内容
> 2、新增了《K8S 部署 CoreDNS 之 DNS 域名获取》文章
...
> 3、新增《K8S 之 DaemonSet》、《K8S 之 Taints（污点）与 Tolerations（容忍）》两篇文章（src/document/Kubernetes）
> 4、两篇新文调整展示状态为展示到博客中
...
## 2026-09-10:<Badge text="安全修复" type="warning" /><Badge text="评论系统更新" type="tip" /><Badge text="评论、浏览量数据重新计算" type="danger" />
> 1、关于本站新增了评论说明，删除了LeanCloud的页面（src/about.md）
> 2、waline文章新增了我的配置（src/document/other/waline-mail.md）
> 3、修复LCP请求（src/README.md）
> 4、说明：更新了评论系统的数据存储（从`LeanCloud`迁移到了`Neon数据库`），所有浏览量和评论数据都会消失，因为没做数据迁移（数据不多），直接更换的；
> 5、祝贺🎉目前评论的等级修复好了，对应的等级如下（随着评论数量而升级）：
